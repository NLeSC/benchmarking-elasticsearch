...
DBFILE = 'queries.db'
DBRESULTSFILE = 'wordcloudshadoop.db'
...
for data in queries:
  ...
  #start = time.time()
  #bin/hadoop jar /home/hkf700/mp/scripts/queries/mapred/mapred.jar mapreduce1.mapred5 /output/mapred5test8 10.149.3.3 "{\"
  ...
  #end = time.time()
  outputfolder = '/output/' + id + '_' + str(data[0])
  q = create_query(data[1], {'lower': data[2], 'upper': data[3]}, params['distributions'].split('+'), params['article_types'].split('+'))
  start = time.time()
  exitCode = subprocess.call(["/local/hkf700/hadoop/bin/hadoop", "jar", "/home/hkf700/mp/scripts/queries/mapred/mapred.jar",
                              "mapreduce1.mapred5", outputfolder, "10.149.3.3", json.dumps(q)])
  end = time.time()
  done += 1
